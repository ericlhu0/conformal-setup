...
def get_single_token_probs(
    model: str, sys_prompt: str, text_input: str, image_input: str
) -> Dict[Any, Any]:
...
if __name__ == "__main__":
    sys_prompt = textwrap.dedent(
        ...
        Expected output:
          - If asked for a single sensitivity value (per body part), return **only** that value in the requested format (e.g., an integer in allowed bounds or a category), with no extra text.
    ...
    text_input = textwrap.dedent(
        """\
        Current action description: You are gently repositioning the user's wrist during a therapy session.
        Current state:
          Contact forces: {{
          "entire_arm": 2,
          "upper_arm": 1,
        ...
        Received feedback:
          ...
          Facial expression: Please examine the provided facial expression image to assess the person's comfort level.
    """
    ).strip()
    #   Facial expression: modality='image', description='assets/faceimgs/high/s001a.jpg'

    prompt = f"\n\nWhat is the updated comfort threshold (1-5 scale) for the wrist? Answer with only a single threshold value."
    ...
    print(
        f"Full output: {get_full_output(model='chatgpt-4o-latest', sys_prompt=sys_prompt, text_input=text_input + prompt, image_input=image_input )}"
    )
